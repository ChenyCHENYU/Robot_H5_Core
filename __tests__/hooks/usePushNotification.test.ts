...
import { createMockBridge, withSetup } from "./_helpers";
...
describe("usePushNotification", () => {
  ...
  it("初始状态正确", () => {
    const { result: { messages, loading, error } } = withSetup(() => usePushNotification());
  ...
  it("register 成功", async () => {
    const { result: { register } } = withSetup(() => usePushNotification());
  ...
  it("register 错误处理", async () => {
    ...
    const { result: { register, error } } = withSetup(() => usePushNotification());
  ...
  it("onMessage 注册回调", () => {
    const { result: { onMessage } } = withSetup(() => usePushNotification());
  ...
  it("clearMessages 清空消息", () => {
    const { result: { messages, clearMessages } } = withSetup(() => usePushNotification());
    messages.value.push({
      title: "test",
      body: "test body",
      timestamp: Date.now(),
